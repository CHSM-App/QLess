  ...
  void _addSlot(int i) => setState(() {
    ...
    if (slots.isNotEmpty) {
      ...
      for (final s in slots) {
        final em = s.endTime.hour * 60 + s.endTime.minute;
        if (em > latestEnd) latestEnd = em;
      }
      int startMin = latestEnd;
      int endMin   = startMin + 180;
      const dayMax = 23 * 60 + 59;
      if (endMin > dayMax) {
        endMin   = dayMax;
        startMin = (endMin - 60).clamp(0, dayMax);
      }
      start = TimeOfDay(hour: startMin ~/ 60, minute: startMin % 60);
      end   = TimeOfDay(hour: endMin   ~/ 60, minute: endMin   % 60);
    }

    _days[i].timeSlots.add(TimeSlot(startTime: start, endTime: end));
  });
  ...
  Future<void> _save() async {
    ...
    for (final day in _days) {
      ...
      for (final slot in day.timeSlots) {
        ...
        if (e <= s) {
          setState(() => day.isExpanded = true);
          _snack('${day.dayName}: end time must be later than start time.',
              isError: true);
          return;
        }
      }
    ...
    // Either a 409 conflict (handle via dialog) or a real error (snackbar).
    final stateNow = ref.read(doctorSettingsViewModelProvider);

    // Hard block: TODAY has bookings on a slot being changed. The weekly
    // editor can't touch today — route the doctor to Leave / the queue.
    final todayMsg = stateNow.todayBlockedMessage;
    if (todayMsg != null) {
      final openLeave = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          ...
          title: Row(
            children: const [
              Icon(Icons.event_busy_rounded, color: kError, size: 22),
              SizedBox(width: 8),
              Expanded(
    ...
    final conflicts = stateNow.pendingConflicts;

    if (conflicts != null && conflicts.isNotEmpty) {
      // 'keep' | 'cancel' | 'reschedule' | null (dismissed = keep)
      final choice = await _showConflictDialog(conflicts);
      if (!mounted) return;

      if (choice == 'cancel' || choice == 'reschedule') {
        final ok2 = await notifier.saveDoctorSchedule(
          model,
          force: true,
          action: choice!,
        );
        ...
        if (ok2) {
          HapticFeedback.lightImpact();
          _snack(choice == 'reschedule'
              ? 'Schedule saved. Patients asked to rebook.'
              : 'Schedule saved. Affected patients have been notified.');
        } else {
          _snack(
            ref.read(doctorSettingsViewModelProvider).errorMessage.isEmpty
                ? 'Failed to save schedule'
                : ref.read(doctorSettingsViewModelProvider).errorMessage,
            isError: true,
          );
        }
      } else {
        notifier.clearConflicts();
      }
      return;
    }

    final err = stateNow.errorMessage;
    _snack(err.isEmpty ? 'Failed to save schedule' : err, isError: true);
  }

  /// Returns one of: 'keep' | 'cancel' | 'reschedule' | null (dismissed).
  Future<String?> _showConflictDialog(List<ScheduleConflict> conflicts) {
    final total = conflicts.fold<int>(0, (s, c) => s + c.futureAppointments);
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              ...
              '${total == 1 ? 'is' : 'are'} booked on slot${conflicts.length == 1 ? '' : 's'} '
              'you are removing or disabling:',
              style: const TextStyle(fontSize: 13, color: kTextPrimary),
            ),
            ...
            ...conflicts.map((c) => Padding(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(
                      child: Text(
                        '${c.day} — ${c.futureAppointments} '
                        'appointment${c.futureAppointments == 1 ? '' : 's'}',
            ...
            const Text(
              'Cancel & continue: appointments are cancelled.\n'
              'Reschedule: patients are asked to book a new time.',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('Cancel & continue'),
          ),
        ],
      ),
    );
