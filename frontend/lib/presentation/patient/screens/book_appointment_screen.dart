...
class BookAppointmentScreen extends ConsumerStatefulWidget {
  ...
  /// When rescheduling, pre-select the original appointment's date so the
  /// patient lands directly on that day.
...
class _BookAppointmentScreenState
  ...
  // Reschedule prefill: original date is applied once availability loads.
  DateTime? _pendingRescheduleDate;
  ...
  void initState() {
    ...
    // Reschedule: remember the original date; it's applied via _pickDate
    // once doctor availability has loaded (see build()).
    final initDate = widget.initialDate;
    if (initDate != null) {
      final now   = DateTime.now();
      final today = DateTime(now.year, now.month, now.day);
      final d     = DateTime(initDate.year, initDate.month, initDate.day);
      if (!d.isBefore(today)) _pendingRescheduleDate = d;
    }
...
void dispose() {
  ...
  Widget build(BuildContext context) {
    ...
    final leaveRanges = state.leaveRanges;

    // Reschedule prefill: once availability is loaded, select the original
    // date exactly as if the patient tapped it (filters sessions, auto-picks
    // a single session, fetches queue estimate).
    if (!_prefillApplied &&
        _pendingRescheduleDate != null &&
        grouped.isNotEmpty) {
      final d = _pendingRescheduleDate!;
      _prefillApplied = true;
      _pendingRescheduleDate = null;
      const wd = ['Monday', 'Tuesday', 'Wednesday', 'Thursday',
                  'Friday', 'Saturday', 'Sunday'];
      final daySessions = grouped[wd[(d.weekday - 1).clamp(0, 6)]] ?? [];
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _pickDate(d, daySessions);
      });
    }
