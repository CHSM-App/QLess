...
class _PatientPrescriptionListScreenState
  ...
  Widget build(BuildContext context) {
    ...
    final all = _filtered(mapped, 'all');
    final active = _filtered(mapped, 'active');
    final past = _filtered(mapped, 'completed') + _filtered(mapped, 'expired');
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Column(
            children: [
              _buildHeader(), // फक्त white bar fixed
              Expanded(
                child: ListView(
      ...
      children: [
        ...
        ListView.separated(
          ...
          separatorBuilder: (_, __) =>
              const Divider(height: 1, color: kBorder, indent: 14),
          itemBuilder: (_, i) {
            final m = _rx.medicines[i];
            final c = _typeColor[m.medicineTypeId] ?? kTextMid;
            final lbl =
                _typeLabel[m.medicineTypeId] ?? m.mediTypeName ?? 'Unknown';
            final ic = _typeIcon[m.medicineTypeId] ?? Icons.medication_rounded;
            return _MedRow(med: m, color: c, typeLabel: lbl, typeIcon: ic);
          },
        ),
      ],
    ),
      ...
      children: [
        Expanded(
          ...
          child: Column(
            ...
            children: [
              Text(
                med.medicineName != null && med.medicineName!.isNotEmpty
                    ? med.medicineName!
              ...
              Container(
                ...
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(typeIcon, color: color, size: 10),
                    const SizedBox(width: 4),
                    Text(
                      typeLabel,
                      style: TextStyle(
