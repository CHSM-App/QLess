...
class _DoctorSettingsPageState extends ConsumerState<DoctorSettingsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isLargeTablet
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                        flex: 3,
                        child: _buildScroll(
                            isTablet: true, s: doctorState, d: doctorDetails)),
          ...
          : _buildScroll(
              isTablet: isTablet, s: doctorState, d: doctorDetails),
    );
  }
  ...
  Widget _buildProfileCard(bool isTablet, DoctorLoginState s, DoctorDetails? d) {
          ...
          child: Column(children: [
            ...
            Transform.translate(
              ...
              child: Column(children: [
                ...
                Text([spec, if (qual.isNotEmpty) qual].join('  •  '),
                    style: const TextStyle(
  ...
  Widget _statItem(String v, String l) => Column(children: [
        ...
        const SizedBox(height: 2),
        Text(l, style: const TextStyle(fontSize: 10, color: kTextMuted)),
      ]);

  Widget _vDiv() => Container(height: 28, width: 1, color: kBorder);
          ...
          Row(children: [
            Container(
              ...
              child: Icon(Icons.videocam_outlined,
                  color: _availableForConsult ? kSuccess : kError, size: 17),
            ),
            ...
            Switch.adaptive(
              value: _availableForConsult, activeColor: kSuccess,
              onChanged: (v) => setState(() => _availableForConsult = v),
            ),
          ]),
          ...
          Column(children: [
            ...
            Row(children: [
              _WheelPicker(
                ...
                value: _leadHours, max: 24,
                onChanged: (v) => _onLeadTimeChanged(v, _leadMinutes),
              ),
              const SizedBox(width: 6),
              const Text(':',
                  style: TextStyle(
              ...
              const SizedBox(width: 6),
              _WheelPicker(
                key: ValueKey('m_${_leadMinutes}_$_savedLeadMinutes'),
                value: _leadMinutes, max: 60,
                onChanged: (v) => _onLeadTimeChanged(_leadHours, v),
              ),
            ]),
          ]),
        ],
  ...
  Widget _dayRow(String day, String hours, bool active) => Row(children: [
        Container(
          width: 7, height: 7,
          decoration: BoxDecoration(
              shape: BoxShape.circle,
  ...
  Widget _buildNotificationsCard() => Container(
        decoration: _cardDec(),
        child: Column(children: [
          _toggleTile(Icons.notifications_outlined, kAmberLight, kWarning,
  ...
  Widget _buildAppearanceCard() => Container(
        ...
        child: Column(children: [
          ...
          const Divider(height: 1, indent: 56, color: kDivider),
          _navTile(_Item(Icons.language_outlined, 'Language', 'English (India)')),
          const Divider(height: 1, indent: 56, color: kDivider),
          _navTile(_Item(Icons.text_fields_outlined, 'Text Size', 'Medium')),
        ]),
      );
  ...
  Widget _healthRow(String label, double value) => Column(
        crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    bool value, ValueChanged<bool> onChanged,
      ...
      Padding(
        ...
        child: Row(children: [
          ...
          Switch.adaptive(
              value: value, activeColor: kPrimary, onChanged: onChanged),
        ]),
      );
...
class _PersonalInfoSheet extends StatelessWidget {
  ...
  const _PersonalInfoSheet({
    ...
    required this.mobile,         required this.specialization,
    required this.qualification,  required this.clinicName,
    required this.experience,     required this.fee,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        _IRow(Icons.phone_outlined,           kSuccess, kGreenLight,   'Mobile',           mobile),
        _IRow(Icons.medical_services_outlined, kPrimary, kPrimaryLight, 'Specialization',   specialization),
        _IRow(Icons.school_outlined,           kPurple,  kPurpleLight,  'Qualification',    qualification),
        _IRow(Icons.local_hospital_outlined,   kWarning, kAmberLight,   'Clinic',           clinicName),
        _IRow(Icons.work_history_outlined,     kSuccess, kGreenLight,   'Experience',
            experience != '—' ? '$experience years' : '—'),
        _IRow(Icons.currency_rupee_outlined,   kWarning, kAmberLight,   'Consultation Fee', fee,
            isLast: true),
...
// ════════════════════════════════════════════════════════════════════
//  PROFESSIONAL DETAILS SHEET
// ════════════════════════════════════════════════════════════════════
class _ProfessionalDetailsSheet extends StatelessWidget {
  final String specialization, qualification, licenseNo, experience, fee,
      clinicName, clinicAddress, clinicEmail, clinicContact, websiteName;

  const _ProfessionalDetailsSheet({
    required this.specialization, required this.qualification,
    required this.licenseNo,      required this.experience,
    required this.fee,            required this.clinicName,
    required this.clinicAddress,  required this.clinicEmail,
    required this.clinicContact,  required this.websiteName,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                  color: kPrimaryLight,
                  borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.badge_outlined, size: 17, color: kPrimary),
            ),
            const SizedBox(width: 10),
            const Expanded(
        ...
        _IRow(Icons.medical_services_outlined, kPrimary,  kPrimaryLight, 'Specialization',   specialization),
        _IRow(Icons.school_outlined,           kPurple,   kPurpleLight,  'Qualification',    qualification),
        _IRow(Icons.verified_outlined,         kSuccess,  kGreenLight,   'License No.',      licenseNo),
        _IRow(Icons.work_history_outlined,     kSuccess,  kGreenLight,   'Experience',       experience),
        _IRow(Icons.currency_rupee_outlined,   kWarning,  kAmberLight,   'Consultation Fee', fee),
        _IRow(Icons.local_hospital_outlined,   kWarning,  kAmberLight,   'Clinic',           clinicName),
        _IRow(Icons.location_on_outlined,      kError,    kRedLight,     'Clinic Address',   clinicAddress),
        _IRow(Icons.email_outlined,            kInfo,     kInfoLight,    'Clinic Email',     clinicEmail),
        _IRow(Icons.phone_outlined,            kSuccess,  kGreenLight,   'Clinic Contact',   clinicContact),
        _IRow(Icons.public_outlined,           kPrimary,  kPrimaryLight, 'Website',          websiteName,
            isLast: true),
...
class _IRow extends StatelessWidget {
  ...
  const _IRow(this.icon, this.iconFg, this.iconBg, this.label, this.value,
      {this.isLast = false});
