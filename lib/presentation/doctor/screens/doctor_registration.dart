...
class _DoctorProfileSetupScreenState
  ...
  // Lead time (global for queue bookings)
  int  _leadHours   = 0;
  int  _leadMinutes = 0;
  ...
  Future<void> _handleSubmit() async {
    ...
    // ── STEP 2 → STEP 3: Register doctor+clinic, capture returned IDs ─────
    if (_step == 2) {
      ...
      await ref
          ...
          .addDoctorDetails(doctorPayload);

      final latestState = ref.read(doctorLoginViewModelProvider);

      // Show error and stay on step 2 if registration failed
      if (latestState.error != null) {
        _showError(latestState.error!);
        return;
      }

      // Read the doctor_id and clinic_id that the ViewModel stored from the
      // API response: { "doctor_id": 42, "clinic_id": "CL001" }
    ...
    // ── STEP 3: Validate schedule → save schedule → save lead time → done ──
    // Overlap validation
    ...
    // Empty-slot validation for enabled days
    ...
    // Save the weekly schedule (uses _savedDoctorId which is now valid)
    await ref
        .read(doctorSettingsViewModelProvider.notifier)
        .saveDoctorSchedule(_buildScheduleModel());

    await ref
         .read(doctorLoginViewModelProvider.notifier).updateLeadTime(_buildLeadTimeModel());
    ...
    // If any queue slots exist, also persist the lead time via updateLeadTime
    if (_anyQueueEnabled && _savedDoctorId != null) {
      ...
      await ref
          ...
          .updateLeadTime(
            DoctorDetails(
              doctorId: _savedDoctorId,   // the real ID returned from Step 2
      ...
      if (leadErr != null) {
        // Non-fatal: schedule saved; just warn the doctor
        _showError('Schedule saved, but lead time update failed: $leadErr');
        // Still navigate — don't block the flow
      }
    }
  ...
  Widget _buildStep3() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Info banner showing captured IDs
        if (_savedDoctorId != null)
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: kSchedPrimary.withOpacity(0.4)),
            ),
            child: Row(
              children: [
                const Icon(Icons.check_circle_rounded, color: kSchedPrimary, size: 16),
        ...
        ..._days.asMap().entries.map((e) => _ScheduleDayCard(
          ...
          onUpdateSlot: (si, updated)=> _updateSlot(e.key, si, updated),
        )),

        // Queue lead time — only visible when any queue slot exists
        AnimatedSize(
          ...
          child: Row(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.access_time_outlined, color: kSchedSuccess, size: 20),
              ),
...
// ═════════════════════════════════════════════════════════════════════════════
// SCHEDULE DAY CARD  (self-contained, mirrors DoctorAvailabilityPage style)
// ═════════════════════════════════════════════════════════════════════════════
        ...
        child: Padding(
          ...
          child: Row(children: [
            Container(
              width: 40, height: 40,
              decoration: BoxDecoration(
                color: schedule.isEnabled ? kSchedPrimaryLight : const Color(0xFFF7F8FA),
                borderRadius: BorderRadius.circular(10),
              ),
              ...
              child: Text(
                ...
                style: TextStyle(
                  ...
                  color: schedule.isEnabled ? kSchedPrimary : kSchedTextMuted,
                ),
              ),
            ),
            const SizedBox(width: 12),
              ...
              children: [
                Text(schedule.dayName,
                    style: TextStyle(
                        ...
                        color: schedule.isEnabled ? kSchedTextPrimary : kSchedTextMuted)),
                Text(
                  schedule.isEnabled
                      ...
                      : 'Unavailable',
                  style: const TextStyle(fontSize: 11, color: kSchedTextMuted),
                ),
              ],
            )),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(
                ...
                side: const BorderSide(color: kSchedPrimary, width: 1.5),
...
class _ScheduleTimeSlotCard extends StatefulWidget {
  ...
  @override
  State<_ScheduleTimeSlotCard> createState() => _ScheduleTimeSlotCardState();
}

class _ScheduleTimeSlotCardState extends State<_ScheduleTimeSlotCard> {
  ...
  bool _overlaps(TimeOfDay start, TimeOfDay end) {
    final s = _toMin(start); final e = _toMin(end);
    ...
    final picked = await showTimePicker(
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(ctx).copyWith(
            colorScheme: const ColorScheme.light(primary: kSchedPrimary)),
        child: child!,
      ),
    );
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(
            width: 20, height: 20,
            decoration: const BoxDecoration(color: kSchedPrimaryLight, shape: BoxShape.circle),
            alignment: Alignment.center,
            child: Text('${widget.index + 1}',
                style: const TextStyle(
                    fontSize: 10, fontWeight: FontWeight.w700, color: kSchedPrimary)),
          ),
          const SizedBox(width: 8),
          const Text('Time Slot',
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: kSchedTextPrimary)),
          const Spacer(),
          GestureDetector(
            ...
            child: Container(
              width: 26, height: 26,
              decoration: BoxDecoration(
                  color: kSchedRedLight, borderRadius: BorderRadius.circular(7)),
              alignment: Alignment.center,
              child: const Icon(Icons.close_rounded, size: 14, color: kSchedError),
            ),
          ),
        ]),
        ...
        Row(children: [
          ...
          Padding(
            ...
            child: Text('→', style: TextStyle(fontSize: 14, color: kSchedTextMuted)),
          ),
          Expanded(child: _SchedTimePicker(
        ...
        const Text('Booking Mode',
            style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                color: kSchedTextSecondary, letterSpacing: 0.2)),
        const SizedBox(height: 6),
        Row(
              ...
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: sel ? kSchedPrimary : kSchedBorder),
                  ),
                  alignment: Alignment.center,
                  child: Text(_modeLabel(mode),
                      style: TextStyle(
                          fontSize: 12, fontWeight: FontWeight.w600,
                          color: sel ? Colors.white : kSchedTextSecondary)),
                ),
              ),
            );
          }).toList(),
        ...
        if (_local.bookingMode == BookingMode.queue ||
          ...
          Container(
            ...
            child: Row(children: [
              const SizedBox(width: 10),
              const Icon(Icons.people_alt_rounded, size: 15, color: kSchedPrimary),
              const SizedBox(width: 8),
              Expanded(
                child: TextField(
                  ...
                  style: const TextStyle(
                      fontSize: 13, fontWeight: FontWeight.w600, color: kSchedTextPrimary),
                  decoration: const InputDecoration(
                    border: InputBorder.none,
                    hintText: 'e.g. 20',
                    hintStyle: TextStyle(fontSize: 13, color: kSchedTextMuted),
                    isDense: true,
              ...
              Container(
                ...
                child: const Text('patients',
                    style: TextStyle(
                        fontSize: 10, fontWeight: FontWeight.w600, color: kSchedPrimary)),
              ),
            ]),
          ),
        ],
...
class _SchedTimePicker extends StatelessWidget {
  ...
  const _SchedTimePicker({required this.label, required this.time, required this.onTap});
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label,
                style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w500,
                    color: kSchedTextMuted, letterSpacing: 0.3)),
            const SizedBox(height: 2),
            Row(children: [
              const Icon(Icons.access_time_rounded, size: 12, color: kSchedPrimary),
...
// ─── Wheel Picker (lead time) ─────────────────────────────────────────────
class _WheelPicker extends StatefulWidget {
  final int value, max;
  final ValueChanged<int> onChanged;
  const _WheelPicker({super.key,
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: ShaderMask(
          ...
          child: ListWheelScrollView.useDelegate(
            itemExtent: 30, diameterRatio: 2.0, perspective: 0.002,
                ...
                return Center(
                  child: AnimatedDefaultTextStyle(
                    ...
                    style: TextStyle(
                      ...
                      color: sel ? kSchedPrimary : kSchedTextMuted,
                    ),
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
        Container(width: 3, height: 14,
            decoration: BoxDecoration(color: kPrimaryBlue, borderRadius: BorderRadius.circular(2))),
        const SizedBox(width: 8),
        Text(title,
            style: const TextStyle(
                fontSize: 13, fontWeight: FontWeight.w600, color: kTextDark, letterSpacing: 0.3)),
      ]);
}
...
class _AvatarPicker extends StatelessWidget {
  ...
  const _AvatarPicker({
    required this.photo, required this.icon,
...
class _StepChip extends StatelessWidget {
  ...
  const _StepChip({required this.label, required this.icon, required this.state});
  ...
  Widget build(BuildContext context) {
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      Container(
        ...
        child: Icon(displayIcon, size: 13, color: fg),
      ),
...
class _MapPickerScreenState extends State<_MapPickerScreen> {
  ...
  @override
  void initState() { super.initState(); _selected = widget.initialLatLng; }
